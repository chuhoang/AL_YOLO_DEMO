...
class Feats_Extraction():
    ...
    def predict_img(self, image_path, diversity=False):
        ...
        for img_path in [image_path]:
            ...
            x = torch.from_numpy(x).to('cuda').float()  # Chuyển đổi từ numpy thành tensor
    ...
    def extract_feature(self, image_path):
        ...
        mlvl_labels = torch.from_numpy(np.array(mlvl_labels[0], dtype=np.float32)).cuda()
        mlvl_scores = torch.from_numpy(np.array(mlvl_scores[0], dtype=np.float32)).cuda()
