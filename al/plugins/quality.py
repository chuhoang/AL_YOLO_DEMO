...
class Quality:
    ...
    def __init__(self, model, base_momentum, img_path):        
        ...
        self.class_quality = torch.zeros((self.num_classes,)).cuda()
        self.class_momentum = torch.ones((self.num_classes,)).cuda() * base_momentum
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
            ...
            if nl == 0:
                ...
                out = torch.zeros(batch_size, counts.max(), ne - 1).cuda()
    ...
    def bbox_decode(self, anchor_points, pred_dist):
                proj=torch.arange(16, dtype=torch.float).cuda()
    ...
    def single_loss(self, model, batch, quality_xi=0.6):
        ...
        imgsz = torch.tensor(feats[0].shape[2:],  dtype=dtype).cuda() * stride[0].to('cuda') # image size (h,w)
        anchor_points, stride_tensor = make_anchors(feats, stride, 0.5)
        anc_points=anchor_points * stride_tensor
        
        # Targets
        targets = torch.cat((batch["batch_idx"].view(-1, 1), batch["cls"].view(-1, 1), batch["bboxes"]), 1).cuda()
